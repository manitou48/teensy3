/*
   Based on:
   https://os.mbed.com/users/kirchnet/code/Nucleo_vs_Arduino_Speed_Test/

   This is the same whetstone code with adjustments for the
   Serial.print function on the Arduino
   The main loop within the procedure whetstone() is identical.
   Only the output format is Arduino-specific.
*/

#define LOOPS  1000   // control duration
#define DO_DOUBLE    // comment out for float
...
/* function prototypes */
void POUT(long N, long J, long K, DBLFLT X1, DBLFLT X2, DBLFLT X3, DBLFLT X4);
void PA(DBLFLT E[]);
void P0(void);
void P3(DBLFLT X, DBLFLT Y, DBLFLT *Z);
...
whetstone(void)
{
  ...
  loopstart = LOOPS;       /* see the note about LOOP below */
  continuous = 0;
...
IILOOP:
  ...
  */
  ...
  for (I = 1; I <= N7; I++) {
    X = T * DATAN(T2 * DSIN(X) * DCOS(X) / (DCOS(X + Y) + DCOS(X - Y) - FCONST1));
    Y = T * DATAN(T2 * DSIN(Y) * DCOS(Y) / (DCOS(X + Y) + DCOS(X - Y) - FCONST1));
  }
  ...
  */
  ...
  if (KIPS >= 1000.0) {
    Serial.print(title);
    Serial.print(" Precision Whetstones: ");
...
#ifdef PRINTOUT
void
POUT(long N, long J, long K, DBLFLT X1, DBLFLT X2, DBLFLT X3, DBLFLT X4)
{
